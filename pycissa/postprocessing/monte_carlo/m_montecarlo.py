...
def run_m_monte_carlo_test(x: np.ndarray,
    ...
    # In M-CiSSA, the results generated by generate_m_results_dictionary nested components differently or directly.
    # The BSS code often accesses `result.get('components', {}).get('trend')` but let's be safe.
    if remove_trend:
        trend_comp = result.get('components', {}).get('trend')
        if trend_comp is not None:
            x_copy -= trend_comp.get('reconstructed_data').reshape(x_copy.shape)
        elif 'mcissa' in result and 'components' in result['mcissa']:
            trend_comp = result['mcissa']['components'].get('trend')
            if trend_comp is not None:
                x_copy -= trend_comp.get('reconstructed_data').reshape(x_copy.shape)
    ...
    if not components and 'mcissa' in result:
        components = result['mcissa'].get('components', {})
    ...
    for surrogate_i in range(0, number_of_surrogates):
        ...
        if remove_trend:
            ...
            if trend_comp is not None:
                x_surrogate += trend_comp.get('reconstructed_data').reshape(x_surrogate.shape)

        pzz_surrogate = m_calculate_surrogate_psd(x_surrogate, L, extension_type, extend_left, extend_right, int(nft))

        for results_key_j in components.keys():
            key_array_position = components.get(results_key_j).get('array_position')
