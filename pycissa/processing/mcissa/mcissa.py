...
class MCissa:
    ...
    def pre_fill_uneven_timeseries(self,
        ...
        joint-channel imputation for optimal accuracy (`multivariate=True`). If set
        to `False`, iterates over each channel independently to fill gaps using univariate
        uneven gap filling.
        '''
        import warnings
        ...
        if multivariate:
            ...
            if plot and 'fig' in res:
                if 'mcissa' not in self.figures:
                    self.figures['mcissa'] = {}
                self.figures['mcissa']['figure_uneven_gap_fill'] = res['fig']

            self.information_text += f'''
        ------------------------------------------------------
        Joint Multivariate uneven gap fill RMSE: {res['rmse']}
        Joint Multivariate uneven gap fill R2: {res['r2']}
        '''
